/**
 * Baglung IUDP — Unified Map Module
 * Consolidates js/map.js (chapter-rail) and spatial.html inline (chip UI).
 * Preserves → consolidate → improve. Feature-detects page UI.
 * Leaflet 1.9.4, no build step. Source: assets/data/*.geojson + assets/overlays/*.png
 * Field-level authority unchanged. No invented layers.
 */
const BOUNDS = {
...
async function initMap(){
  ...
  if(hasRail) L.control.zoom({position:'bottomright'}).addTo(map);
  ...
  fetch('assets/data/roads.geojson').then(r=>r.json()).then(roads=>{
    vectorLayers.roads = L.geoJSON(roads,{pane:'vectorPane',style:roadStyle,onEachFeature:(f,l)=>{
      const p=f.properties;
      l.bindPopup(`<b>${p.road_category||'Road'}</b><br>${p.name||''}<br>Ward ${p.WARD||'—'} · ${(p.length_km||0).toFixed(2)} km<br>Surface: ${p.surface_class||'—'}`);
    }});
  ...
  if(sel){
    [...wards.features].sort((a,b)=>a.properties.WARD-b.properties.WARD).forEach(f=>{
      const o=document.createElement('option'); o.value=f.properties.WARD; o.textContent=`Ward ${f.properties.WARD} — ${f.properties.POPULATION?.toLocaleString()} ppl`; sel.appendChild(o);
    });
  ...
  // wire UI: detect which controls exist
  wireControls(); // handles data-base/overlay/vector, opacity, drawer, legend
  wireChips(); // handles simple [data-layer] chips (spatial.html) — no-op if none
  wireOpacity();
  buildLegend();
  if(hasRail){
    applyScene('overview');
...
// ── Controls (chapter-rail / advanced panel) ───────────────────────────────
function wireControls(){
  ...
  if(ctrlTrigger && ctrlPanel){
    ...
    ctrlTrigger.textContent='⚙ Layers';
    ...
    ctrlTrigger.addEventListener('click',()=>{
      ...
      ctrlTrigger.textContent = willOpen ? '✕ Close' : '⚙ Layers';
  ...
  if(legendTrigger && legendEl){
    legendTrigger?.addEventListener('click',()=>{
      ...
      if(willOpen && ctrlTrigger) ctrlTrigger.textContent='⚙ Layers';
    });
    legendClose?.addEventListener('click',()=>{
      ...
      if(isMob){
        ...
        legendClose.textContent = hidden ? '−' : '+';
      }
    });
  }
  // hazard toggle
...
function buildLegend(){
  // support both ids: #legendBody (advanced) and #mapLegend (simple)
  const el=document.getElementById('legendBody') || document.getElementById('mapLegend');
  if(!el) return;
  const active = Object.entries(state.overlays).filter(([k,v])=>v).map(([k])=>k);
  let html='';
  if(active.includes('dem')) html+=`<div class="row"><span class="sw" style="background:linear-gradient(90deg,#2e7d32,#ffea00,#c1121f)"></span> DEM 645–2679 m (terrain tint)</div>`;
  if(active.includes('dem')||active.includes('hillshade') && !active.includes('dem')) html+=`<div class="row"><span class="sw" style="background:linear-gradient(90deg,#2e7d32,#ffea00,#c1121f)"></span> DEM 645–2679 m</div>`;
  ...
  if(active.includes('flood')||active.includes('landslide')) html+=`<div class="row"><span class="sw" style="background:linear-gradient(90deg,#fff8ef,#ffd166,#ef476f,#7f1d1d)"></span> Risk low → high</div>`;
  if(!html){
    // default legend (no overlay active)
    const isSimple = !!document.querySelector('[data-layer]');
    if(isSimple){
      html=`<div class="row"><span class="sw" style="background:#f2a900"></span> Ward pop: 1 875–9 829</div><div class="row"><span class="sw" style="background:#c1121f"></span> Hospital / Primary road</div><div class="row"><span class="sw" style="background:#1c4f3a"></span> School</div>`;
    } else {
      html=`<div class="row"><span class="sw" style="background:#f2a900"></span> Ward pop: 1 875–9 829</div><div class="row"><span class="sw" style="background:#c1121f"></span> Primary road</div><div class="row"><span class="sw" style="background:#1c4f3a"></span> School / Hospital (tap)</div>`;
    }
  }
  if(el.id==='mapLegend'){
    el.innerHTML='<h5>Legend</h5>'+html;
  } else {
    el.innerHTML=html;
    const legendEl=document.getElementById('legend');
...
// ── Simple chip UI (spatial.html) ───────────────────────────────────────────
function wireChips(){
  const chips = document.querySelectorAll('[data-layer]');
  if(!chips.length) return;
  chips.forEach(chip=>{
    chip.addEventListener('click',()=>{
      const name=chip.getAttribute('data-layer');
      const isWards = name.startsWith('wards');
      const key = isWards ? 'wards' : name;
      const isOverlay = OVERLAYS.hasOwnProperty(key);
      const on = isOverlay ? !state.overlays[key] : !state.vectors[key];
      if(isOverlay){
        const lyr=imageLayers[key];
        if(!lyr) return;
        state.overlays[key]=on;
        if(on){ lyr.setOpacity(0); lyr.addTo(map); requestAnimationFrame(()=>lyr.setOpacity(OVERLAYS[key].opacity)); }
        else { lyr.setOpacity(0); setTimeout(()=>{if(!state.overlays[key]&&map.hasLayer(lyr)) map.removeLayer(lyr);},220); }
        chip.classList.toggle('active', on);
      } else {
        const lyr=vectorLayers[key];
        if(!lyr) return;
        state.vectors[key]=on;
        if(on){ if(!map.hasLayer(lyr)) lyr.addTo(map); } else { if(map.hasLayer(lyr)) map.removeLayer(lyr); }
        chip.classList.toggle('active', on);
      }
      buildLegend();
    });
...
function wireOpacity(){
  const op=document.getElementById('opacity'); if(!op)return;
  op.addEventListener('input',()=>{
    const v=parseFloat(op.value);
    Object.values(imageLayers).forEach(l=>l.setOpacity(v));
    const lbl=document.getElementById('opVal'); if(lbl) lbl.textContent=Math.round(v*100)+'%';
  });
}
...
function applyScene(id){
  ...
  for(const k of Object.keys(OVERLAYS)){
    ...
    // also sync chips
    const chip=document.querySelector(`[data-layer="${k}"]`);
    if(chip) chip.classList.toggle('active', want);
  ...
  for(const k of Object.keys(vectorLayers)){
    ...
    if(cb) cb.checked=want;
    const chip=document.querySelector(`[data-layer="${k}"]`)||document.querySelector(`[data-layer="${k} active"]`);
    if(chip) chip.classList.toggle('active', want);
